...
meanv=traffspeed(tf_x:end,tf_y); % 平均车速（×公交车缩小系数） 剩余edge*1 -- kph
...
f=fit(Pdis, sq, 'smoothingspline', 'SmoothingParam', 3.3465206225961505E-5);
...
%     else%只需要平均功率
        CDRpos(tt)=f(PowerPre(tt));
        CDRMethod(tf_x+tt-1)=2;
%     end
end
CDRpos(CDRpos<-4.7e-9)=-4.7e-9;
