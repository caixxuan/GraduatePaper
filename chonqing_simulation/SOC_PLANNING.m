...
tf_y=fix((time+502)/300);%ty_y为当前时间time前的上一采样时间节点
tf_x=edge+1;%初始edge==1,tf_x为当前所在edge
meang=GradeEdge(tf_x:end);

%通过历史信息来预测未来的车速和功率分布
[meanv,Pdmd_distribution]=TraffEst(trafficspeed,RefSpdLmt,VelDis,tf_x,tf_y,distance_edge,meang);

Froad=0.8*7.67.*(meanv*3.6).^2/21.15+14500*9.8*0.012.*cos(meang)+14500*9.8.*sin(meang);
...
PowerPre=Froad.*meanv/ave_EFF/1000;%预测剩余edge的功率需求kW
if PMcon<60%判断需要功率分布信息的路段
    DisturbRange=[PMcon-20:60+20]';
else
...
for tt=1:n
    if PowerPre(tt)>min(DisturbRange) && PowerPre(tt)<max(DisturbRange)%需要功率分布的路段
        CDRpos(tt)=sum( sq.*Pdmd_distribution(tt,:)' )/sum( Pdmd_distribution(tt,:) );
